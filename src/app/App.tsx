...
import {EmblaCarousel} from "../components/NewCarousel/Carousel.tsx"
...
const tabs = [
  ...
  },
  {
    ...
    icon: <svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fillRule="evenodd" clipRule="evenodd"
              d="M8.38137 0.860764C8.59804 0.692245 8.90144 0.692245 9.1181 0.860764L15.1181 5.52743C15.2643 5.6411 15.3497 5.81589 15.3497 6.00104V13.3344C15.3497 13.8471 15.146 14.3389 14.7835 14.7014C14.4209 15.064 13.9292 15.2677 13.4164 15.2677H4.08307C3.57032 15.2677 3.07857 15.064 2.716 14.7014C2.35343 14.3389 2.14974 13.8471 2.14974 13.3344V6.00104C2.14974 5.81589 2.23522 5.6411 2.38137 5.52743L8.38137 0.860764ZM3.34974 6.29449V13.3344C3.34974 13.5289 3.427 13.7154 3.56453 13.8529C3.70205 13.9904 3.88858 14.0677 4.08307 14.0677H6.14974V8.00078C6.14974 7.66941 6.41837 7.40078 6.74974 7.40078H10.7497C11.0811 7.40078 11.3497 7.66941 11.3497 8.00078V14.0677H13.4164C13.6109 14.0677 13.7974 13.9904 13.935 13.8529C14.0725 13.7154 14.1497 13.5289 14.1497 13.3344V6.29449L8.74974 2.09449L3.34974 6.29449ZM10.1497 14.0677V8.60078H7.34974V14.0677H10.1497Z"
              fill="white" fillOpacity="0.6"/>
      </svg>,
    slides: [
      <img src={img2} alt="img"/>,
...
const Slide:FC<{children: ReactNode}> = ({children}) => (
  <div className={s.slide} style={{width: `100%`}}>
    {children}
  </div>
)
  ...
  return (
    <main className={s.main}>
      <div className={s.tabsGroup}>
        {tabs.map((el, index) => (
          <div key={index} className={s.tab + (index === activeTab ? ' ' + s.active : '')} onClick={() => setActiveTab(index)}>
            {el.icon}
            {el.name}
          </div>
        ))}
      </div>
      <EmblaCarousel key={activeTab} activeTab={activeTab} onSlideCantScrollNext={() => increment()} onSlideCantScrollPrev={() => decrement()}>
        {tabs[activeTab].slides.map((el, index) => <Slide key={index}>{el}</Slide>)}
      </EmblaCarousel>
    </main>
  )
}
